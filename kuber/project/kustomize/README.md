...
Every environment is an overlay of [overlays](./overlays), chosen with `-o`
and defaulting to `production`:
...
```shell
./release.sh version -o dev      # bump the dev version, commit it
./release.sh install -o dev      # deploy it
../../apply-all.sh -o dev        # the same, plus the secrets
```
...
up by the folder name, nothing else has to be edited.
...
```shell
./release.sh version                    # production, version of build.gradle
./release.sh version -t 1.9.7-SNAPSHOT  # or an explicit tag
./release.sh version -o dev             # another environment
...
./release.sh install -o dev                 # another environment
